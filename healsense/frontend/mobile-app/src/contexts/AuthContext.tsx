...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const initializeAuth = async () => {
    try {
      ...
      if (!existingUsers) {
        await AsyncStorage.setItem(MOCK_USERS_KEY, JSON.stringify([DEFAULT_MOCK_USER]));
      }
      ...
      if (token) {
        ...
        const users = JSON.parse(await AsyncStorage.getItem(MOCK_USERS_KEY) || '[]');
  ...
  const login = async (email: string, password: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      // Save auth token
      await AsyncStorage.setItem(AUTH_TOKEN_KEY, JSON.stringify({ userId: foundUser.id }));
  ...
  const signup = async (
    ...
    try {
      ...
      await AsyncStorage.setItem(AUTH_TOKEN_KEY, JSON.stringify({ userId: newUser.id }));
